...
<body>
    <?php if (!empty($rutinas)): ?>
        ...
        <center>
            <h1>Rutinas Gratuitas</h1>
        </center>

        <div class="contenedor-rutinas">
            <?php foreach ($rutinas as $rutina): ?>
                <?php if ($rutina['tipo_rutina'] == 0): ?>
                    <div class="card">
                        <h2>
                            <?= esc($rutina['nombre_rutina']) ?>
                        </h2>
                        <p><strong>Descripcion:</strong>
                            <?= esc($rutina['descripcion']) ?>
                        </p>
                        <p><strong>Duración:</strong>
                            <?= esc($rutina['duracion']) ?>
                        </p>
                        <p><strong>Dificultad:</strong>
                            <?= esc($rutina['dificultad']) ?>
                        </p>
                        <!-- Número de descargas entre paréntesis -->
                        <span class="descargas"><strong>Descargas:</strong> (
                            <?= esc($rutina['descargas']) ?>)
                        </span>
